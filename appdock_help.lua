...
<p>Unterstützt werden serverseitig geliefertes HTML, relative Links, kurze lokale Historie und statische PNG-, JPEG-, GIF- oder WebP-Bilder. Bilder werden nur über HTTP(S) mit begrenzter Größe in einen temporären lokalen Cache geladen; nicht verfügbare oder nicht unterstützte Bilder erscheinen als Alt-Text. Bewusst nicht unterstützt werden JavaScript, WebSockets, eingebettete Medien, Formulare, Downloads, SVG und Nicht-HTTP(S)-Links. Das ist eine Sicherheits- und Lesbarkeitsentscheidung, kein Defekt.</p>
...
<p>It supports server-rendered HTML, relative links, short local history and static PNG, JPEG, GIF or WebP images. Images are fetched only over HTTP(S) with bounded size into a temporary local cache; unavailable or unsupported images appear as alt text. JavaScript, WebSockets, embedded media, forms, downloads, SVG and non-HTTP(S) links are deliberately unsupported. This is a security and readability decision, not a defect.</p>
